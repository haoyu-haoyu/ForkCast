...
8. Blind backtest: make this the high point. Show R1-R6 from `backtest_result.json`, with R1 as PARTIAL and R6 as BALANCED HIT. Show the prompt transparency panel and "No outcome tokens in prompt".
...
- Backtest credibility comes from `blind_prediction.json`, not mock simulation events.
...
- The demo uses one historical case, so describe it as answer-isolated historical backtesting, not randomized holdout validation.
...
- The full path Before -> ULEZ selected -> extraction approval -> agent approval -> run approval -> replay -> report -> blind backtest -> Audit + Kaspa -> close was clickable on the local Vite dashboard.
- The blind backtest table rendered the real R1-R6 result set. R1 showed PARTIAL; R6 showed BALANCED HIT.
